...
if ( ! class_exists( 'Alg_WC_Checkout_Fees' ) ) :
	...
	/**
	 * Calculate the fees on checkout page.
	 */
	class Alg_WC_Checkout_Fees {
		...
		/**
		 * WC_Square_Payment_Form_args
		 *
		 * @var $wc_square_card_payment_args
		 */
		public $wc_square_card_payment_args = '';
		...
		public function __construct() {
			if ( 'yes' === get_option( 'alg_woocommerce_checkout_fees_enabled', 'yes' ) ) {
				...
				add_action( 'wp_enqueue_scripts', array( $this, 'enqueue_checkout_script' ), 15 );
				...
				// exclude taxes from discount when fees value are kept negative.
				add_action( 'woocommerce_cart_totals_get_fees_from_cart_taxes', array( $this, 'alg_wc_checkout_fees_exclude_cart_fees_taxes' ), 10, 3 );

				// Check if WooCommerce Square Payment Plugin is activated and insert fix.
				if ( in_array( 'woocommerce-square/woocommerce-square.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ), true ) ) {
					add_filter( 'wc_square_credit_card_payment_form_js_args', array( $this, 'alg_wc_set_wc_square_card_payment_args' ), 10, 2 );
				}
			}
		}
		...
		public function convert_currency( $amount ) {
			...
			return apply_filters( 'wc_aelia_cs_convert_pgbf', $amount, $this->base_currency, $this->current_currency );
		}
		 ...
		 * global fees only
		 *
		 * @param string $current_gateway Currenct selected gateway.
		 * @param string $fee_num Fee number 1/2.
		 * @version 2.5.1
		 * @since   2.0.0
		 */
		public function check_countries( $current_gateway, $fee_num = '' ) {
			...
			if ( ! empty( $include_countries ) && ! in_array( $customer_country, $include_countries ) ) { //phpcs:ignore
				return false;
			}
			...
			if ( ! empty( $exclude_countries ) && in_array( $customer_country, $exclude_countries ) ) { //phpcs:ignore
				return false;
			}
			if ( '' !== $fee_num ) {
				...
				if ( ! empty( $include_states ) && ! in_array( $customer_state, $include_states ) ) { //phpcs:ignore
					return false;
				}
				...
				if ( ! empty( $exclude_states ) && in_array( $customer_state, $exclude_states ) ) { //phpcs:ignore
					return false;
				}
			}
			return true;
		...
		public function register_script() {
			wp_register_script(
				...
				trailingslashit( plugin_dir_url( __DIR__ ) ) . 'assets/js/checkout-fees.js',
				array( 'jquery' ),
				alg_wc_cf()->version,
				true
			);
		...
		public function enqueue_checkout_script() {
			...
			wp_localize_script(
				...
				array(
					...
					'alg_wc_square_card_payment_args' => $this->wc_square_card_payment_args,
				)
			);
		}

		/**
		 * Get currenct selected gateway.
		 *
		 * @version 2.5.4
		 * @since   2.4.0
		 */
		public function get_current_gateway() {
			...
			if ( '' === $current_gateway ) {
				$current_gateway = ( ! empty( sanitize_key( $_REQUEST['payment_method'] ) ) ? sanitize_key( $_REQUEST['payment_method'] ) : '' );
				if ( '' === $current_gateway ) {
		...
		public function add_gateways_fees( $the_cart ) {
			...
			if ( in_array( 'woocommerce-subscriptions/woocommerce-subscriptions.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) { //phpcs:ignore
				...
				if ( $cart_contains_subscription && ( count( $this->fees_added ) > 0 || count( $this->fees_added_2 ) > 0 ) && ( ( is_checkout() && ( empty( $_POST['woocommerce-process-checkout-nonce'] ) || ! wp_verify_nonce( sanitize_key( $_POST['woocommerce-process-checkout-nonce'] ), 'woocommerce-process_checkout' ) ) ) || is_cart() ) ) {
			...
			if ( 'yes' === get_option( 'alg_woocommerce_checkout_fees_global_fee_enabled', 'no' ) ) {
				...
				if ( $do_add ) {
					...
					if ( ! empty( $gateways_excl ) && in_array( $current_gateway, $gateways_excl ) ) { //phpcs:ignore
						$do_add = false;
					}
				}
				//Remove the global fee when min and max values are set
				if ( $do_add ) {
					$min_num = get_option( 'alg_woocommerce_checkout_fees_golbal_min_fees', 0 );
					$max_num = get_option( 'alg_woocommerce_checkout_fees_golbal_max_fees', 0 );
					$sub_total = WC()->cart->get_subtotal();
					if(($sub_total >= $min_num) && ($sub_total<= $max_num)){
						$do_add= false;
						}
					}
				if ( $do_add ) {
		...
		public function calculate_the_fee( $args, $final_fee_to_add, $total_in_cart, $fee_num ) {
			...
			switch ( $fee_type ) {
				...
				case 'percent':
					foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
						if ( $cart_item['product_id'] == $args['product_id'] ) {
							$p_price = $cart_item['data']->get_price();
						}
					}
					if ( 0 != $args['product_id'] ) { //phpcs:ignore
						$_product    = wc_get_product( $args['product_id'] );
						$sum_for_fee = $p_price * $args['product_qty'];
					} else {
						$sum_for_fee = $total_in_cart;
					}
					if ( in_array( 'woo-wallet/woo-wallet.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ), true ) ) {
						$fees = WC()->cart->get_fees();
						if ( isset( $fees['_via_wallet_partial_payment']->amount ) ) {
							$wallet_amount = $fees['_via_wallet_partial_payment']->amount;
							$sum_for_fee   = $sum_for_fee - abs( $wallet_amount );
						}
					}
			...
			// Max fee.
			if ( $max_fee != '' && 0 != $max_fee && $new_fee > $max_fee ) { //phpcs:ignore
				$new_fee = $max_fee;
			}
			// Max total discount.
		 ...
		 * @param string  $fee_num Fee number.
		 * @param float   $total_in_cart Total fees in cart.
		 * @param bool    $is_info_only if the fees is info only.
		 * @param integer $info_product_id Product ID.
		 * @version 2.5.3
		 * @since   1.2.0
		...
		public function get_the_fee( $args, $fee_num, $total_in_cart = 0, $is_info_only = false, $info_product_id = 0 ) {
			...
			if ( '' !== $args['current_gateway'] && 'yes' === $args['is_enabled'] ) {
				...
				if ( $total_in_cart >= $args['min_cart_amount'] && ( '' == $args['max_cart_amount'] || 0 == $args['max_cart_amount'] || $total_in_cart <= $args['max_cart_amount'] ) ) { //phpcs:ignore
		...
		public function modify_fee_html_for_taxes( $cart_fee_html, $fees ) {
			...
			if ( 'incl' === get_option( 'woocommerce_tax_display_cart' ) && isset( $fees->tax ) && $fees->tax > 0 && ( in_array( $fees->name, $this->fees_added ) || in_array( $fees->name, $this->fees_added_2 ) ) ) { //phpcs:ignore
				$hide_include_tax_text = apply_filters( 'alg_wc_checkout_fees_hide_include_tax_text', false );
				if ( ! $hide_include_tax_text ) {
		...
		/**
		 * Function to exclude taxes from discount when the fees are kept negative as when the Tax were kept inclusive fees were coming with the tax included in it.
		 *
		 * @param array  $taxes Array of taxes.
		 * @param object $fee Object of fees.
		 * @param object $cart Cart object.
		 */
		public function alg_wc_checkout_fees_exclude_cart_fees_taxes( $taxes, $fee, $cart ) {
			foreach ( $fee as $key => $value ) {
				// if fees are kept negative it will return and not add tax in the fees.
				if ( isset( $value->amount ) ) {
					if ( $value->amount == 0 ) { //phpcs:ignore
						return array();
					}
				}
			}
		...
		public function modify_stripe_params( $stripe_params ) {
			global $wp;
			if ( strpos( $wp->request, 'add-payment-method' ) !== false ) {
				$stripe_params['is_checkout'] = 'no';
			} else {
		...
		/**
		 * Function to retrieve/save the args used by the WC Square Payment Plugin.
		 *
		 * @param array $args Array of arguments for WC Square Payment Plugin.
		 * @param Payment_Form $instance payment form instance.
		 */
		public function alg_wc_set_wc_square_card_payment_args( $args, $instance ) {
			$this->wc_square_card_payment_args = $args;
			return $args;
		}
	}
