 ...
 *
 * @package checkout-fees-for-woocommerce-pro/frontend
 */
...
if ( ! class_exists( 'Alg_WC_Checkout_Fees_Info' ) ) :
	...
	class Alg_WC_Checkout_Fees_Info {
		...
		/**
		 * Show full info on product page.
		 *
		 * @version 2.0.0
		 * @since   2.0.0
		 */
		...
		/**
		 * Get checkout fees info.
		 *
		 * @param bool $lowest_price_only display on lowest price or not.
		 * @version 2.5.0
		 * @since   1.2.0
		 */
		public function get_checkout_fees_info( $lowest_price_only ) {
			...
			foreach ( $products_array as $product_data ) {
				...
				foreach ( $wc_available_gateways as $available_gateway_keys => $wc_available_gateway ) {
					$globally_fees_enabled = get_option( 'alg_gateways_fees_enabled_' . $available_gateway_keys );
					$per_product_fees      = get_post_meta( $product_id, '_alg_checkout_fees_enabled_' . $available_gateway_keys, true );
					if ( 'yes' === $globally_fees_enabled || 'yes' === $per_product_fees ) {
						$available_gateways[ $available_gateway_keys ] = $wc_available_gateway;
					}
				}
