...
if ( ! class_exists( 'Alg_WC_Order_Fees' ) ) :
	...
	class Alg_WC_Order_Fees {
		...
		public function __construct() {
			...
			add_action( 'woocommerce_order_item_add_action_buttons', array( $this, 'alg_wc_pg_checkbox' ) );
			add_action( 'admin_enqueue_scripts', array( $this, 'alg_wc_cf_register_script' ) );
			add_action( 'wp_ajax_admin_fees_checkbox', array( $this, 'wp_ajax_admin_fees_checkbox_callback' ) );
			add_action( 'woocommerce_saved_order_items', array( $this, 'alg_wc_cf_update_order_fees' ), PHP_INT_MAX, 1 );
		}
		 ...
		 */
		public function alg_wc_cf_update_order_fees( $order_id ) {
			$order = wc_get_order( $order_id );
			if ( $this->pgbf_wc_hpos_enabled() ) {
				$fees_checkbox_value = $order->get_meta( 'alg_wc_cf_add_fees_value' );
			} else {
				$fees_checkbox_value = get_post_meta( $order_id, 'alg_wc_cf_add_fees_value', true );
			}
			$payment_method = $order->get_payment_method();
			if ( 'on' === $fees_checkbox_value && '' !== $payment_method ) {
				$this->remove_fees( $order );
				$this->add_gateways_fees( $order, $payment_method );
			}
			if ( $this->pgbf_wc_hpos_enabled() ) {
				$order->update_meta_data( 'alg_wc_cf_add_fees_value', $fees_checkbox_value );
				$order->save();
			} else {
				update_post_meta( $order_id, 'alg_wc_cf_add_fees_value', $fees_checkbox_value );
			}
		}
		/**
		...
		/**
		 * Function to add checkbox for adding fees in the backend.
		 *
		 * @param object $order Order object.
		 */
		public function alg_wc_pg_checkbox( $order ) {
			...
			$checked = '';
			if ( 'on' === $fees_checkbox_value ) {
				$checked = 'checked';
			}
			?>
			<input type = "checkbox" name = "alg_wc_cf_add_fees" id="alg_wc_cf_add_fees" <?php echo esc_attr( $checked ); ?> >
			<?php
				echo esc_attr_e( 'Apply payment gateway fees.', 'checkout-fees-for-woocommerce' );
			?>
			</select>
			<?php
		}
		...
		/**
		 * Include JS files on add/edit order page.
		 */
		public function alg_wc_cf_register_script() {
			if ( is_admin() ) {
				...
				if ( is_admin() && 'shop_order' === $screen_type || 'woocommerce_page_wc-orders' === $screen_type && 'new' === $action || 'edit' === $action ) {
					...
					wp_register_script(
						...
						trailingslashit( plugin_dir_url( __DIR__ ) ) . 'assets/js/admin-order-fees.js',
						array( 'jquery' ),
						alg_wc_cf()->version,
						true
					);
					wp_enqueue_script( 'alg-payment-gateways-admin-order-page' );
					wp_localize_script(
						'alg-payment-gateways-admin-order-page',
						'admin_order_fees_checkbox',
						array(
							'ajax_url'                  => admin_url( 'admin-ajax.php' ),
							'admin_fees_checkbox_nonce' => wp_create_nonce( 'admin-fees-checkbox' ),
							'order_id'                  => $order_id,
						)
					);
				}
		...
		/**
		 * Callback function of admin fees checkbox to update the checkbox value.
		 */
		public function wp_ajax_admin_fees_checkbox_callback() {
			global $wp;
			check_ajax_referer( 'admin-fees-checkbox', 'security' );
			if ( isset( $_POST['checkbox_value'] ) && isset( $_POST['orderid'] ) ) {
				$checkbox_value = sanitize_text_field( wp_unslash( $_POST['checkbox_value'] ) );
				$post_id        = sanitize_text_field( wp_unslash( $_POST['orderid'] ) );
				if ( $this->pgbf_wc_hpos_enabled() ) {
					$order = wc_get_order( $post_id );
					$order->update_meta_data( 'alg_wc_cf_add_fees_value', $checkbox_value );
					$order->save();
				} else {
					update_post_meta( $post_id, 'alg_wc_cf_add_fees_value', $checkbox_value );
				}
			}
		}
			...
			if ( 0 != $final_fee_to_add || 0 != $final_fee_to_add_2 ) { //phpcs:ignore
				...
				if ( 0 != $final_fee_to_add ) { //phpcs:ignore
					if ( $this->do_merge_fees ) {
						...
						$item_fee->set_tax_class( $tax_class_slug ); // default for empty tax.
				...
				case 'percent':
					if ( 0 != $args['product_id'] ) { //phpcs:ignore
						...
						if ( (float) 0 === $total_in_cart ) {
							...
							$sum_for_fee = $total_in_cart;
							$due_fee     = $order->get_fees();
							if ( ! empty( $due_fee ) ) {
								foreach ( $due_fee as $fee_id => $fee ) {
									// Get total.
									$sum_for_fee += $fee['line_total'];
								}
							}
						}
					}
