<?php
/**
 * Payment Gateway Based Fees and Discounts for WooCommerce - Data Tracking Class
 *
 * @since 2.6.3
 *
 * @package Payment Gateway Based Fees and Discounts for WooCommerce/Data Tracking
...
if ( ! class_exists( 'Cf_Data_Tracking' ) ) :

	/**
	 * Payment Gateway Based Fees and Discounts Data Tracking Core.
	 */
	class Cf_Data_Tracking {
		...
		public function __construct() {
			...
			// Track user's choice.
			add_action( 'wp_ajax_cf_pro_admin_choice', array( __CLASS__, 'cf_pro_admin_choice' ) );
			// Add a recurring As action.
			add_action( 'init', array( __CLASS__, 'ts_add_recurring_action' ) );
			// Send Tracker Data.
			add_action( 'ts_send_data_tracking_usage', array( __CLASS__, 'cf_pro_send_tracking_data' ) );
		}
		...
		public static function ts_admin_notice() {
			...
			$plugin_url  = plugins_url() . '/checkout-fees-for-woocommerce-pro';
			$plugin_name = 'Payment Gateway Based Fees and Discounts for WooCommerce';
			$doc_link    = 'https://www.tychesoftwares.com/docs/docs/payment-gateway-based-fees-and-discounts-for-woocommerce/payment-gateway-based-fees-and-discounts-usage-tracking/';

			// Condition to check if user has already made a choice.
			if ( '' === get_option( 'cf_pro_allow_tracking', '' ) ) {
				?>
				<div class=''>
					<div class="cf-pro-message cf-pro-tracker notice notice-info is-dismissible" style="position: relative;">
							...
							<?php
								printf(
									wp_kses_post(
										...
										__( 'Want to help make %1$s even more awesome? Allow %1$s to collect non-sensitive diagnostic data and usage information and get 20%% off on your next purchase. <a href="%2$s" target="_blank">Find out more</a>.', 'checkout-fees-for-woocommerce' )
									),
						...
						<p class="submit">
							<a class="button-primary button button-large" id="cf-pro-allow" href="<?php echo esc_url( add_query_arg( 'cf_pro_tracker_optin', 'true' ) ); ?>"><?php esc_html_e( 'Allow', 'checkout-fees-for-woocommerce' ); ?></a>
							<a class="button-secondary button button-large skip" id="cf-pro-disallow" href="<?php echo esc_url( add_query_arg( 'cf_pro_tracker_optout', 'true' ) ); ?>"><?php esc_html_e( 'No thanks', 'checkout-fees-for-woocommerce' ); ?></a>
						</p>
					</div>
				</div>
				<?php
		...
		public static function ts_admin_notices_scripts() {
			...
			if ( '' === get_option( 'cf_pro_allow_tracking', '' ) ) {
				$plugin_url   = plugins_url() . '/checkout-fees-for-woocommerce-pro';
				$cf_instance = alg_wc_cf();

				wp_enqueue_script(
					'cf_dismiss_notice',
					$plugin_url . '/assets/js/checkout-fees-tracking-notice.js',
					'',
					$cf_instance->version,
					false
				);
		...
		public static function ts_update_tracking() {
			...
			if ( current_user_can( 'administrator' ) ) {
				...
				if ( isset( $_GET['cf_pro_tracker_optin'] ) ) { //phpcs:ignore WordPress.Security.NonceVerification
					update_option( 'cf_pro_allow_tracking', 'yes' );
					if ( '' !== $url ) {
						header( "Location: $url" );
					}
				} elseif ( isset( $_GET['cf_pro_tracker_optout'] ) ) { //phpcs:ignore WordPress.Security.NonceVerification
					update_option( 'cf_pro_allow_tracking', 'no' );
					// Run a single call to send this to our servers.
					self::cf_pro_send_tracking_data();
					// Reload the page.
					if ( '' !== $url ) {
						header( "Location: $url" );
					}
		 ...
		 * @since 2.6.3
		 */
		public static function cf_pro_admin_choice() {
			...
			update_option( 'cf_pro_allow_tracking', $admin_choice );
			// Run a single call to send this to our servers.
			self::cf_pro_send_tracking_data();
		}
		...
		public static function ts_add_recurring_action() {
			if ( function_exists( 'as_next_scheduled_action' ) ) { // Indicates that the AS library is present.
				if ( false === as_next_scheduled_action( 'ts_send_data_tracking_usage' ) && 'yes' === get_option( 'cf_pro_allow_tracking', '' ) ) {
		 ...
		 * @since 2.6.3
		 */
		public static function cf_pro_send_tracking_data() {

			$allow_tracking = get_option( 'cf_pro_allow_tracking', '' );
		...
		public static function ts_get_tracking_data() {
			...
			$data = Cf_Tracking_Functions::cf_pro_plugin_tracking_data( $data );
...
$cf_data_tracking = new Cf_Data_Tracking();
