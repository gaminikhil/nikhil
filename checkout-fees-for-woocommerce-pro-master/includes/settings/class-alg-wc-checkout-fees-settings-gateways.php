 ...
 *
 * @package checkout-fees-for-woocommerce-pro/settings/global
 */
...
if ( ! class_exists( 'Alg_WC_Checkout_Fees_Settings_Gateways' ) ) :
	...
	/**
	 * Global settings for each gateway.
	 */
	class Alg_WC_Checkout_Fees_Settings_Gateways {
		...
		/**
		 * Create a settings section for each gateways.
		 *
		...
		public function get_settings() {
			...
			$pro_version_link = sprintf( __( 'You will need <a target="_blank" href="%s">Pro version</a> of the plugin to set this option.', 'checkout-fees-for-woocommerce' ), 'https://wpfactory.com/item/payment-gateway-based-fees-and-discounts-for-woocommerce-plugin/' );
			...
			$plugin_name = 'bykea-cash-online-payments/online-payments-bykeacash.php';
			if ( in_array( $plugin_name, apply_filters( 'active_plugins', get_option( 'active_plugins', array() ) ), true ) ) {
				?>
				<style type="text/css">
				.woocommerce-save-button.hide-save-btn {
