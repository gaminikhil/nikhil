...
if ( ! class_exists( 'Cf_Tracking_Functions' ) ) :
	...
	class Cf_Tracking_Functions {
		 ...
		 * @since 2.6.3
		 */
		public static function cf_pro_plugin_tracking_data( $data ) {
			...
			$plugin_data = array(
				...
				'license_data'              => self::cf_get_license_data(),
				'enabled_gateways_for_fees' => self::cf_get_enabled_gateways_for_fees(),
				'product_count'             => self::cf_get_product_count(),
			);
		 ...
		 * @since 2.6.3
		 */
		public static function cf_get_enabled_gateways_for_fees() {

			$active_gateways      = self::wc_payment_gateways();
			$active_gateways_fees = array();

			foreach ( $active_gateways as $key => $value ) {
				$args                            = alg_wc_cf()->core->args_manager->get_the_args_global( $key );
				$args['countries_include_fee_1'] = get_option( 'alg_gateways_fees_countries_include_fee_1_' . $key );
				$args['countries_exclude_fee_1'] = get_option( 'alg_gateways_fees_countries_exclude_fee_1_' . $key );
				$args['states_include_fee_1']    = get_option( 'alg_gateways_fees_states_include_fee_1_' . $key );
				$args['states_exclude_fee_1']    = get_option( 'alg_gateways_fees_states_exclude_fee_1_' . $key );
				$args['cats_include']            = get_option( 'alg_gateways_fees_cats_include_' . $key );
				$args['cats_exclude']            = get_option( 'alg_gateways_fees_cats_exclude_' . $key );
				$args['countries_include_fee_2'] = get_option( 'alg_gateways_fees_countries_include_fee_2_' . $key );
				$args['countries_exclude_fee_2'] = get_option( 'alg_gateways_fees_countries_exclude_fee_2_' . $key );
				$args['states_include_fee_2']    = get_option( 'alg_gateways_fees_states_include_fee_2_' . $key );
				$args['states_exclude_fee_2']    = get_option( 'alg_gateways_fees_states_exclude_fee_2_' . $key );
				$args['cats_include_fee_2']      = get_option( 'alg_gateways_fees_cats_include_fee_2_' . $key );
				$args['cats_exclude_fee_2']      = get_option( 'alg_gateways_fees_cats_exclude_fee_2_' . $key );
				$args['countries_include']       = get_option( 'alg_gateways_fees_countries_include_' . $key );
				$args['countries_exclude']       = get_option( 'alg_gateways_fees_countries_exclude_' . $key );
				$args['cats_include_calc_type']  = get_option( 'alg_gateways_fees_cats_include_calc_type_' . $key );
				$args['cats_exclude_calc_type']  = get_option( 'alg_gateways_fees_cats_exclude_calc_type_' . $key );
		...
		/**
		 * Fetch Number of Products in which Gateways settings are enabled
		 *
		 * @since 2.6.3
		 */
		public static function cf_get_product_count() {
		 ...
		 * @since 2.6.3
		 */
		public static function cf_get_license_data() {

			return wp_json_encode(
				array(
					'license_key'    => get_option( 'edd_license_key_cf' ),
					'license_status' => get_option( 'edd_license_key_cf_status' ),
				)
			);
		}
	}
